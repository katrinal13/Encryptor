public class Encryptor
{
  ...
  public String encryptMessage(String message)
  {
      ...
      int j = 0;
      for (int i = 1; i <= (int) (((double) length / (numRows * numCols)) + 0.5); i++)
      {
          System.out.println(i);
          if (i != (int) (((double) length / (numRows * numCols)) + 0.5))
          {
              fillBlock(message.substring(j, j + (numRows * numCols)));
          }
          else
          {
              fillBlock(message.substring(j, message.length()));
          }
